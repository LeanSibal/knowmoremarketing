...
function genesis_sample_woocommerce_theme_notice() {
	...
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'know-more-marketing' ), esc_html( CHILD_THEME_NAME ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		$install_link = sprintf(
			...
			), __( 'install and activate Genesis Connect for WooCommerce', 'know-more-marketing' )
		);

		/* translators: 1: plugin install prompt presented as link, 2: child theme name */
		$notice_html = sprintf( __( 'Please %1$s to <strong>enable WooCommerce support for %2$s</strong>.', 'know-more-marketing' ), $install_link, esc_html( CHILD_THEME_NAME ) );
	}

	echo '<div class="notice notice-info is-dismissible know-more-marketing-woocommerce-notice"><p>' . wp_kses_post( $notice_html ) . '</p></div>';
